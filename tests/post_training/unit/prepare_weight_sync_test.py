...
class PrepareWeightSyncTest(unittest.TestCase):
  ...
  def test_single_synchronizer_creation_and_binding(self, mock_sync_cls):
    ...
    self.engine._checkpoint_manager.wait_until_finished.assert_called_once()
  ...
  def test_drain_before_convert_and_single_tree_peak_liveness(self):
    ...
    # wait_until_finished always runs before convert
    self.assertLess(events.index("wait_until_finished"), events.index("convert"))
