...
"""Unit and golden parity tests for Cosmos 3 layers.
...
from maxtext.models import cosmos3
import numpy as np

# Module-level aliases for cosmos3 attention and layer symbols.
Cosmos3Config = cosmos3.Cosmos3Config
Cosmos3MLP = cosmos3.Cosmos3MLP
Cosmos3MoTDecoderLayer = cosmos3.Cosmos3MoTDecoderLayer
Cosmos3Sharding = cosmos3.Cosmos3Sharding
Cosmos3ShardingHook = cosmos3.Cosmos3ShardingHook
CosmosAttention = cosmos3.CosmosAttention
CosmosDualAttention = cosmos3.CosmosDualAttention
apply_rotary_pos_emb = cosmos3.apply_rotary_pos_emb
build_causal_understanding_mask = cosmos3.build_causal_understanding_mask
build_causal_understanding_splash_mask = cosmos3.build_causal_understanding_splash_mask
build_cosmos_packing_metadata = cosmos3.build_cosmos_packing_metadata
build_full_generative_mask = cosmos3.build_full_generative_mask
build_full_generative_splash_mask = cosmos3.build_full_generative_splash_mask
causal_understanding_attention = cosmos3.causal_understanding_attention
compile_cosmos_splash_mask = cosmos3.compile_cosmos_splash_mask
compute_3d_mrope_cos_sin = cosmos3.compute_3d_mrope_cos_sin
full_generative_attention = cosmos3.full_generative_attention
reinterleave_streams = cosmos3.reinterleave_streams
unpack_streams = cosmos3.unpack_streams

_ATTENTION_GOLDEN_FILENAME = "cosmos_attention_golden_data.npz"
_MOT_GOLDEN_FILENAME = "cosmos3_mot_golden_data.npz"
...
class Cosmos3MLPTest(parameterized.TestCase):
  ...
  def test_output_shape(self, hidden_act):
    ...
    mlp = Cosmos3MLP(hidden_size=32, intermediate_size=64, hidden_act=hidden_act, rngs=nnx.Rngs(0))
  ...
  def test_relu2_has_no_gate_projection(self):
    ...
    mlp = Cosmos3MLP(hidden_size=32, intermediate_size=64, hidden_act="relu2", rngs=nnx.Rngs(0))
    self.assertIsNone(mlp.gate_proj)

  def test_silu_has_gate_projection(self):
    """Verifies the SwiGLU variant is gated."""
    mlp = Cosmos3MLP(hidden_size=32, intermediate_size=64, hidden_act="silu", rngs=nnx.Rngs(0))
    self.assertIsNotNone(mlp.gate_proj)

  def test_rejects_unknown_activation(self):
    """Verifies unsupported activations are rejected at construction."""
    with self.assertRaises(ValueError):
      Cosmos3MLP(hidden_size=32, intermediate_size=64, hidden_act="gelu", rngs=nnx.Rngs(0))


class Cosmos3MoTDecoderLayerStructureTest(parameterized.TestCase):
  ...
  def _build(self, **overrides):
    ...
    return Cosmos3MoTDecoderLayer(**kwargs)

  def test_diffusers_aliases_share_attention_weights(self):
    """Verifies the diffusers-canonical names alias the attention submodule."""
    layer = self._build()
  ...
  def test_output_shapes(self):
    ...
    layer = self._build()
    meta = build_cosmos_packing_metadata([8], [12])
  ...
  def test_config_and_sharding(self):
    """Verifies config-driven construction and Cosmos3Sharding logical axis rules."""
    cfg = Cosmos3Config(
        hidden_size=64,
        head_dim=16,
        num_attention_heads=4,
    ...
    sharding_rules = Cosmos3Sharding()
    hook = Cosmos3ShardingHook(sharding_rules)
    layer = Cosmos3MoTDecoderLayer.from_config(cfg, rngs=nnx.Rngs(0), sharding_hook=hook)
    meta = build_cosmos_packing_metadata([4], [6])
    ...
    self.assertEqual(
        sharding_rules.init_weight_spec("q_proj", ("embed",), ("heads", "head_dim")),
        jax.sharding.PartitionSpec(("fsdp", "fsdp_t"), ("tp", "tp_t")),
    )
...
class Cosmos3MoTDecoderLayerGoldenParityTest(parameterized.TestCase):
  """Numerical parity against the upstream diffusers reference layer."""
  ...
  def _load_weights(self, layer, data, case_name, dtype):
    """Copies diffusers parameters into the MaxText layer.

    PyTorch ``nn.Linear`` stores ``(out_features, in_features)`` while the Flax
    ``DenseGeneral`` kernel is ``(in_features, out_features)``, so every
    ...
    # Attention projections, addressed through the diffusers-canonical aliases.
    ...
    set_scale(layer.post_attention_layernorm_moe_gen, "post_attention_layernorm_moe_gen.weight")
  ...
  def _run_parity_check(self, case_name: str, precision: str):
    """Compares MaxText outputs against the diffusers golden activations."""
    assert self.golden_data is not None
    data = dict(self.golden_data)
    ...
    meta = build_cosmos_packing_metadata([und_len], [gen_len])
    ...
    print(
        f"\n[Cosmos3MoT Parity: {case_name} | {precision}]"
    ...
    np.testing.assert_allclose(
        und_arr,
        golden_und,
        err_msg=f"und_out differs from diffusers reference in {case_name} ({precision})",
        **tol,
    )
    np.testing.assert_allclose(
        gen_arr,
        golden_gen,
        err_msg=f"gen_out differs from diffusers reference in {case_name} ({precision})",
        **tol,
    )
  ...
  def test_golden_parity(self, case_name, precision):
    """Checks numerical parity against the diffusers golden reference."""
