...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def save_checkpoint(self, metadata: Any, **kwargs: Any) -> None:
    ...
    ckpt_saved = self._checkpoint_manager.save_checkpoint(
        ...
        checkpoint_state=checkpointing.CheckpointState(
            model=self.model,
            optimizer=self.optimizer if self._config.save_optimizer_state else None,
  ...
  def prepare_weight_sync(
    ...
    if effective_transport in ("raiden", "gcs"):
      ...
      # 1. Drain in-flight checkpoint persistence and TPU computations, and drop
      # any leftover converted tree from the previous cycle BEFORE convert().
  ...
  def _purge_raiden_buffers(self) -> int:
    ...
    if self._raiden_sync is None or not hasattr(self._raiden_sync, "release_buffers"):
      return 0
