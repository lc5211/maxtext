...
def DEEPSEEK_V4_MAXTEXT_TO_HF_PARAM_MAPPING(config, maxtext_config, scan_layers=False):
  ...
  for j in range(mtp_num_layers):
    ...
    # The MTP transformer layer is built as DeepSeek4DecoderLayer(layer_idx=0,
    # compress_ratio=0, is_hash_routing=False), so it has neither a compressor nor
    # hash routing, but it does carry a MoE gate bias.
    _add_layer(
